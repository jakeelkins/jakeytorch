...
int main(){
    ...
    for (unsigned int i = 0; i < batch_size*N*M; ++i){
    ...
    for (unsigned int i = 0; i < batch_size*M; ++i){
        x[i] = ((float) rand()) / (float) RAND_MAX;
    }
    
    _batched_gemv(y_ref.data(), A.data(), x.data(), N, M, batch_size);
    
    printf("y_ref: \n");
    batched_print_matrix(y_ref, N, 1, batch_size);
    ...
    checkCudaErrors(cudaMemcpy(y.data(), y_device, byteSize_y, cudaMemcpyDeviceToHost));
    
    printf("y: \n");
    batched_print_matrix(y, N, 1, batch_size);
